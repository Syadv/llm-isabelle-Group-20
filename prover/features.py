...
def goal_op_density(goal):
    if not goal:
        return 0.0
    count = 0
    for op in _OP_TOKENS:
        count = count + goal.count(op)
    return count / len(goal)

def tactic_repeat(candidate, prefix):
    for step in prefix:
        if step == candidate:
            return 1
    return 0

def prefix_diversity(prefix):
    if len(prefix) == 0:
        return 0.0
    seen = []
    for step in prefix:
...
def make_dataset(attempts_paths: List[str]) -> Tuple[List[List[float]], List[int]]:
    ...
    for rec in iter_attempt_rows(paths):
        ...
        if idx is not None:
            step_one_hot[idx] = 1
        repeat = tactic_repeat(cand, prefix)
        diversity = prefix_diversity(prefix)
        op_density = goal_op_density(goal)
        goal_length = len(goal)
        row = [
            depth, n_sub, elapsed, cache_hit,
            flags["is_listy"], flags["is_natty"], flags["is_sety"], flags["has_q"], flags["is_bool"],
        ] + step_one_hot + [len(cand), goal_length, op_density, repeat, diversity]
        X.append(row)
        y.append(ok)
    return X, y
...
def feature_names() -> List[str]:
    base = [
        "depth", "n_sub", "elapsed_ms", "cache_hit",
        "is_listy", "is_natty", "is_sety", "has_q", "is_bool",
    ]
    names = base + [f"step::{t}" for t in STEP_TYPES] + [
        "len_cmd", "goal_length", "op_density", "tactic_repeat", "prefix_diversity",
    ]
    names += [
        "premise_cosine_top1",
